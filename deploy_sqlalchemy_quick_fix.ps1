# Deploy Quick SQLAlchemy Fix to EC2
# Fixes the _static_cache_key error by upgrading SQLAlchemy

Write-Host "🚀 Deploying Quick SQLAlchemy Fix to EC2..." -ForegroundColor Green

# Configuration
$EC2_HOST = "34.202.215.209"
$EC2_USER = "ubuntu"
$REMOTE_DIR = "/home/ubuntu/ai-backend-python"
$LOCAL_SCRIPT = "fix_sqlalchemy_quick.py"

Write-Host "📦 Copying SQLAlchemy fix script to EC2..." -ForegroundColor Yellow

# Copy the fix script to EC2
$scpCommand = "scp -i `"C:\projects\lvl_up\New.pem`" -o StrictHostKeyChecking=no $LOCAL_SCRIPT ${EC2_USER}@${EC2_HOST}:${REMOTE_DIR}/"
...
echo "🔧 Quick SQLAlchemy Fix for _static_cache_key Error"
echo "=================================================="
...
$sshCommand = "ssh -i `"C:\projects\lvl_up\New.pem`" -o StrictHostKeyChecking=no ${EC2_USER}@${EC2_HOST} '$sshCommands'"
...
Write-Host "🔗 You can monitor the service with: ssh -i `"C:\projects\lvl_up\New.pem`" ${EC2_USER}@${EC2_HOST} 'sudo journalctl -u ultimate_start -f'" -ForegroundColor Cyan 