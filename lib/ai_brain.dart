...
class TheImperium {
  ...
  final StreamController<Map<String, dynamic>> _metaLearningController =
  ...
  Stream<Map<String, dynamic>> get metaLearningStream =>
      _metaLearningController.stream;
  Stream<bool> get runningStream => _runningController.stream;
  List<Map<String, dynamic>> get metaLearningLog =>
      List.unmodifiable(_metaLearningLog);
  ...
  void start() {
    ...
    print(
      '🏆 The Imperium: Started. Directive: Constant growth, self-learning, and improvement of all AIs.',
    );
  ...
  // Start background loop for continuous improvement (reduced frequency)
    _backgroundLoop ??= Timer.periodic(
      const Duration(minutes: 5),
      (_) => _runMetaImprovementCycle(),
    );
  ...
  // Listen to MissionProvider's learning log (if available)
    ...
    _logMetaEvent(
      'The Imperium started monitoring all AIs and connected to backend.',
    );
    startFileWatchers();
  }

  void _connectToBackendWS() {
    print('[IMPERIUM] WebSocket endpoints not available, skipping connection');
    // WebSocket endpoints are not working on the server, so skip connection
    print('[IMPERIUM] ✅ WebSocket connection skipped (using HTTP fallback)');
  }
  ...
  void _handleWSMessage(dynamic data) {
    try {
      ...
      switch (type) {
        case 'imperium_update':
          _handleImperiumUpdate(message);
          break;
        ...
        case 'proposal:applied':
          _logMetaEvent('Backend event: $type - ${message.toString()}');
          break;
        default:
          print('[IMPERIUM] ⚠️ Unknown message type: $type');
      }
    } catch (e) {
      print('[IMPERIUM] ❌ Error parsing WebSocket message: $e');
    }
  }

  void _handleImperiumUpdate(Map<String, dynamic> message) {
  // Custom logic for Imperium updates
    print('[IMPERIUM] 📨 Imperium update: ${message.toString()}');
    _logMetaEvent('Imperium update: ${message.toString()}');
  }
  ...
  // Simple meta-learning: log and look for repeated issues
    final insight =
        'Analyzed event: ${event['action']} on ${event['files']?.join(', ') ?? 'unknown files'}';
  ...
  void listenToOtherAIs({
    Stream<Map<String, dynamic>>? mechanicumLog,
    Stream<Map<String, dynamic>>? sandboxLog,
  }) {
  ...
  // Propose an improvement
          proposeImprovement(
            'AI Guardian',
            'Repeated issue: $action',
            code: '/ TODO: Auto-generated fix for $action',
          );
        }
      }
    }
  ...
  // Use MissionProvider to apply code (simulate for now)
      MissionProvider.latestInstance?.applyAISuggestionFromImperium(
        suggestion,
        code,
      );
  ...
  void _runMetaImprovementCycle() {
    ...
    print(
      '🏆 The Imperium: [${now.toIso8601String()}] Meta-improvement cycle running. Log entries: ${_metaLearningLog.length}',
    );
  ...
  // Analyze backend activity patterns
    ...
    print(
      '🏆 The Imperium: Found ${backendEvents.length} backend events in log',
    );
  ...
  // Analyze for improvements based on backend activity
    ...
    for (final entry in backendEvents) {
      ...
      if (insight.contains('disconnected')) {
        issueCounts['connection_issues'] =
            (issueCounts['connection_issues'] ?? 0) + 1;
      }
      if (insight.contains('proposal created')) {
        issueCounts['proposals_created'] =
            (issueCounts['proposals_created'] ?? 0) + 1;
      }
    }

  // Generate insights based on backend activity
    if (issueCounts['test_failures'] != null &&
        issueCounts['test_failures']! > 2) {
      _logMetaEvent(
        '🏆 The Imperium: Detected repeated test failures. Recommending code review improvements.',
      );
      proposeImprovement(
        'Backend AIs',
        'Improve test reliability',
        code: '/ TODO: Enhanced error handling and validation',
      );
    }

    if (issueCounts['connection_issues'] != null &&
        issueCounts['connection_issues']! > 1) {
      _logMetaEvent(
        '🏆 The Imperium: Detected connection issues. Recommending network resilience improvements.',
      );
      proposeImprovement(
        'Backend AIs',
        'Improve network resilience',
        code: '/ TODO: Add retry logic and connection pooling',
      );
    }

    if (issueCounts['proposals_created'] != null &&
        issueCounts['proposals_created']! > 5) {
      _logMetaEvent(
        '🏆 The Imperium: High proposal activity detected. System is actively improving.',
      );
    }

    final afterCount = _metaLearningLog.length;
    print(
      '🏆 The Imperium: [${now.toIso8601String()}] Cycle complete. Backend events analyzed: ${backendEvents.length}. Total log entries: $afterCount.',
    );
  }
  ...
  void learnFromExternalCode({
    ...
    final learning = {
      ...
      'insight':
          'Learned from $source. The Imperium adapts and grows faster than other AIs.',
    };
    _metaLearningLog.add(learning);
    _metaLearningController.add(learning);
    print(
      '🏆 The Imperium: [${now.toIso8601String()}] Learned from $source. Code: ${code.substring(0, code.length > 60 ? 60 : code.length)}...',
    );
  }
  ...
  void proposeAndApplyCodeChange({
    ...
    proposeImprovement(
      aiName,
      'Proposed code change to $filePath',
      code: newCode,
    );
    if (autoApply) {
  ...
  // Ensure Conquest AI starts automatically
  static void ensureConquestStarted() {
    print('TheImperium: ensureConquestStarted() called');
  // Initialize Conquest AI provider if needed
    try {
      final context = navigatorKey.currentContext;
      if (context != null) {
        Provider.of<ConquestAIProvider>(context, listen: false);
        print('TheImperium: Conquest AI provider initialized');
      }
    } catch (e) {
      print('TheImperium: Error initializing Conquest AI: $e');
    }
  }
  ...
  // Try parent directory
      ...
      if (Directory('$parentDir/lib').existsSync() &&
        ...
        print(
          'The Imperium: Could not find lib and assets directories, skipping file watchers',
        );
        return;
      }
    }

    print('The Imperium: Using lib path: $libPath');
    print('The Imperium: Using assets path: $assetsPath');
  ...
  // Watch /lib
    try {
      ...
      print('The Imperium: File watchers started for $libPath');
    } catch (e) {
      print('The Imperium: Error starting lib file watcher: $e');
    }
  ...
  // Watch /assets
    try {
      ...
      print('The Imperium: File watchers started for $assetsPath');
    } catch (e) {
      print('The Imperium: Error starting assets file watcher: $e');
    }
  ...
  // Example: If file contains TODO, propose improvement
      if (content.contains('TODO')) {
        ...
        if (filePath.endsWith('ai_brain.dart') ||
            filePath.endsWith('ai_file_system_helper.dart')) {
  ...
  // (In real use, generate and apply the fix here)
          ...
          notifyUser(
            'The Imperium',
            'Auto-applied self-improvement to $filePath',
            icon: '🏆',
          );
        } else {
  // Propose improvement to the provider
          ...
          if (context != null) {
            ...
            notifyUser(
              'The Imperium',
              'New proposal submitted for your review.',
              icon: '🏆',
            );
          }
        }
  ...
  Future<void> _checkAssetFile(String filePath) async {
    try {
      AIFileSystemHelper.logFileAction(
        ...
        'asset_check',
        filePath,
        details: 'Checking asset validity',
      );
      final file = File(filePath);
      if (!file.existsSync()) {
