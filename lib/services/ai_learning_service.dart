...
class AILearningService {
  ...
  Stream<Map<String, dynamic>> get learningUpdateStream => _learningUpdateController.stream;
  Stream<String> get aiLearningEventStream => _aiLearningEventController.stream;
  Stream<Map<String, dynamic>> get debugOutputStream => _debugOutputController.stream;

  /// Initialize the learning service
  Future<void> initialize() async {
    print('[AI_LEARNING_SERVICE] 🧠 Initializing in-app AI learning service...');
  ...
  Future<void> _loadLearningData() async {
    try {
      ...
      if (learningJson != null) {
        ...
        print('[AI_LEARNING_SERVICE] 📚 No existing learning data found, using defaults');
      }
    } catch (e) {
  ...
  Future<void> learnFromProposal(dynamic proposal, String action, String? feedbackReason) async {
    print('[AI_LEARNING_SERVICE] 🧠 Learning from proposal ${proposal.id} - Action: $action');
    ...
    final learningEntry = {
      ...
      'improvementType': 'general', // Default since AIProposal doesn't have this field
    ...
    if (_learningData.containsKey(aiType)) {
      ...
      // Add debug log entry
      _addDebugLog(aiType, 'Learned from $action: ${feedbackReason?.substring(0, 50) ?? 'No feedback'}');
  ...
  Future<void> learnFromBackendTestResult(String aiType, Map<String, dynamic> testData) async {
    print('[AI_LEARNING_SERVICE] 🧪 Learning from backend test result - $aiType');
    ...
    if (_learningData.containsKey(aiType)) {
      ...
      // Add debug log entry
      _addDebugLog(aiType, 'Backend test ${testEntry['result']}: ${testEntry['testType']} - ${testEntry['details'].substring(0, 50)}');
      ...
      print('[AI_LEARNING_SERVICE] ✅ ${aiType} learned from backend test result');
    }
  }
  ...
  Future<void> learnFromTestResult(String aiType, String testType, String result, String details) async {
    print('[AI_LEARNING_SERVICE] 🧪 Learning from test result - $aiType: $testType = $result');
    ...
    if (_learningData.containsKey(aiType)) {
      ...
      // Add debug log entry
      _addDebugLog(aiType, 'Local test $result: $testType - ${details.substring(0, 50)}');
  ...
  void _extractSuccessPatterns(String aiType, Map<String, dynamic> learningEntry) {
    ...
    if (!patterns.any((p) => p['pattern'] == improvementPattern)) {
      ...
      final existing = patterns.firstWhere((p) => p['pattern'] == improvementPattern);
      existing['count'] = (existing['count'] as int) + 1;
      existing['lastSeen'] = learningEntry['timestamp'];
    }
  }
  ...
  void _extractFailurePatterns(String aiType, Map<String, dynamic> learningEntry) {
    final patterns = _learningData[aiType]!['failurePatterns'] as List;
    final feedback = learningEntry['feedbackReason']?.toString().toLowerCase() ?? '';
    
    // Extract common failure patterns
    List<String> failurePatterns = [];
    
    if (feedback.contains('compilation error') || feedback.contains('syntax error')) {
    ...
    for (final pattern in failurePatterns) {
      if (!patterns.any((p) => p['pattern'] == pattern)) {
        patterns.add({
          ...
          'count': 1,
          'lastSeen': learningEntry['timestamp'],
          'examples': [feedback.substring(0, feedback.length > 100 ? 100 : feedback.length)],
        });
      } else {
        ...
        if (existing['examples'] is List) {
          existing['examples'].add(feedback.substring(0, feedback.length > 100 ? 100 : feedback.length));
  ...
  /// Extract lessons from test failures
  void _extractTestFailureLessons(String aiType, Map<String, dynamic> testEntry) {
  ...
  void _extractBackendTestFailureLessons(String aiType, Map<String, dynamic> testEntry) {
    final lessons = _learningData[aiType]!['lessons'] as List;
    final details = testEntry['details'].toString().toLowerCase();
    ...
    } else if (testType.contains('dependency') || details.contains('version solving')) {
      lesson = 'Verify package dependencies are compatible and available';
    } else if (testType.contains('null_safety') || details.contains('null check')) {
    ...
    } else if (testType.contains('security') || details.contains('vulnerability')) {
  ...
  void _extractBackendTestSuccessPatterns(String aiType, Map<String, dynamic> testEntry) {
    final patterns = _learningData[aiType]!['successPatterns'] as List;
    ...
    if (!patterns.any((p) => p['pattern'] == successPattern)) {
      ...
      final existing = patterns.firstWhere((p) => p['pattern'] == successPattern);
      existing['count'] = (existing['count'] as int) + 1;
  ...
  void _showLearningNotification(String aiType, String action, String? feedbackReason) {
    String body;
    if (action == 'approved') {
      body = 'Learned from approved proposal: ${feedbackReason?.substring(0, 50) ?? 'Good work!'}';
    } else if (action == 'rejected') {
      body = 'Learned to avoid: ${feedbackReason?.substring(0, 50) ?? 'User rejection'}';
    } else {
      body = 'Learned from failure: ${feedbackReason?.substring(0, 50) ?? 'Test failed'}';
    }
  ...
  void _showTestNotification(String aiType, String testType, String result, String details) {
    final body = '$testType test: ${details.substring(0, 50)}...';
  ...
  void _showBackendTestNotification(String aiType, Map<String, dynamic> testEntry) {
    final result = testEntry['result'];
    final testType = testEntry['testType'];
    final details = testEntry['details'];
    
    final body = '$testType: ${details.substring(0, 50)}...';
  ...
  Future<void> _calculateLearningScores() async {
    for (final aiType in _learningData.keys) {
      ...
      final recentFailures = (data['userFeedback'] as List)
          .where((f) => f['action'] == 'rejected' || f['action'] == 'test-failed')
          .take(10)
          .length;
      ...
      final backendSuccessRate = backendTests.isEmpty ? 0 : 
          backendTests.where((t) => t['result'] == 'pass').length / backendTests.length;
  ...
  Map<String, dynamic> getLearningMetrics() {
    ...
    for (final aiType in _learningData.keys) {
      ...
      metrics[aiType] = {
        ...
        'failedProposals': (data['userFeedback'] as List)
            .where((f) => f['action'] == 'rejected' || f['action'] == 'test-failed')
            .length,
        'passedProposals': (data['userFeedback'] as List)
  ...
  int _calculateAppliedLearning(Map<String, dynamic> data) {
    ...
    for (final lesson in lessons.take(5)) {
      ...
      final avoidsMistake = recentFeedback.every((feedback) {
        final feedbackText = feedback['feedbackReason']?.toString().toLowerCase() ?? '';
  ...
  List<Map<String, dynamic>> getDebugLog(String aiType) {
    return List<Map<String, dynamic>>.from(_learningData[aiType]?['debugLog'] ?? []);
  }
  ...
  void _notifyLearningUpdate(String aiType, Map<String, dynamic> learningEntry) {
    ...
    _aiLearningEventController.add('$aiType learned from ${learningEntry['action'] ?? 'interaction'}');
  }
  ...
  Future<Map<String, dynamic>> triggerLearningCycle(String aiType, String proposalId, String result) async {
    try {
      final response = await _dio.post('/api/proposals/trigger-learning', data: {
        'aiType': aiType,
  ...
  Future<Map<String, dynamic>> getLearningCycleStats({String? aiType, int days = 30}) async {
    try {
      ...
      final response = await _dio.get('/api/proposals/learning-cycle-stats', queryParameters: queryParams);
      return response.data;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> getGitHubStatus() async {
    try {
      final response = await _dio.get('/api/proposals/github-status');
      return response.data;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> mergeLearningPR(String prUrl) async {
    try {
      final response = await _dio.post('/api/proposals/merge-learning-pr', data: {
        'prUrl': prUrl,
      });
  ...
  Future<Map<String, dynamic>> getInternetLearningStatus({String? aiType}) async {
    try {
      ...
      final response = await _dio.get('/api/proposals/internet-learning-status', queryParameters: queryParams);
      return response.data;
    } catch (e) {
      print('[AI_LEARNING_SERVICE] ❌ Error getting internet learning status: $e');
      return {};
    }
  }
} 