...
class AILearningService:
    ...
    # Feedback loop stub for code proposals
    ...
    async def save_internet_learning_result(self, ai_type: str, topic: str, result: dict, context: str = None):
        """Persist an internet-based learning result to the Learning table"""
        from sqlalchemy import select
        ...
        try:
            learning_entry = Learning(
                ai_type=ai_type,
                learning_type="internet_learning",
                pattern=result.get("title", topic),
                context=result.get("summary", context),
                feedback=result.get("url", None),
                confidence=1.0,  # Could be improved with scoring
                created_at=datetime.utcnow()
            )
            session.add(learning_entry)
            await session.commit()
        except Exception as e:
            logger.error("Error saving internet learning result", error=str(e), ai_type=ai_type, topic=topic)
        finally:
            await session.close() 