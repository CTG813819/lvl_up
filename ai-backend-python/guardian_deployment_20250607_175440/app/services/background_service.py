...
class BackgroundService:
    ...
    async def _imperium_audit_task(self):
        """Run a full backend (and optionally frontend) check every 2 hours, log to Codex, trigger notifications."""
        while self._running:
            try:
                logger.info("🛡️ Imperium AI running full system audit...")
                # Simulate backend check (replace with actual check logic or subprocess call if needed)
                ...
                # Wait 2 hours before next audit
    ...
    async def _run_backend_audit(self):
        """Run backend check and return results (simulate or call script)."""
        # Here you could call the test_backend_comprehensive_check.py script or replicate its logic
    ...
    def _analyze_audit_results(self, results):
        ...
        if results.get('all_ok', True):
            ...
            return ['Review failed endpoints and logs.']
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    try:
                        ...
                        healthy = False
                # 2. Check resource usage (simulate for now)
                # TODO: Implement real resource checks (CPU, memory, disk)
                resource_ok = True
