...
# Fixes port conflicts, missing commands, and service configuration
...
# Step 5: Check for any remaining processes
echo "🔍 Checking for remaining processes..."
ps aux | grep -E "(uvicorn|python.*app.main)" | grep -v grep || echo "  ✅ No remaining processes found"

# Step 6: Update service configuration with proper commands
...
ExecStartPre=/bin/bash -c 'pgrep -f uvicorn | xargs -r kill -9 || true'
