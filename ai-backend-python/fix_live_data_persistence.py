...
async def fix_live_data_persistence():
    """Fix Project Warmaster service to persist live data in Neon database"""
    
    print("🔧 Fixing Project Warmaster Service to Persist Live Data in Neon Database...")
    ...
    try:
        ...
        # Create a new version with database persistence
        persistent_service_content = '''import asyncio
import random
import time
import json
import logging
...
class ProjectWarmasterService:
    """Live Project Warmaster Service with persistent database storage"""
    ...
    async def _load_persistent_data(self, db: AsyncSession):
        ...
        try:
            ...
            self._current_learning_progress = system.learning_progress or 0.0
            self._current_neural_connections = system.neural_connections or 0
            self._current_knowledge_base_size = system.knowledge_base_size or 0
            self._current_capabilities = {
            ...
            print(f"📊 Loaded persistent data: {self._current_learning_progress:.2f} progress, {self._current_neural_connections} connections, {self._current_knowledge_base_size} knowledge")
    ...
    async def _save_persistent_data(self, db: AsyncSession):
        ...
        try:
            ...
            system.learning_progress = self._current_learning_progress
            system.neural_connections = self._current_neural_connections
            system.knowledge_base_size = self._current_knowledge_base_size
            system.nlp_capability = self._current_capabilities["nlp_capability"]
            system.voice_interaction = self._current_capabilities["voice_interaction"]
            system.device_control = self._current_capabilities["device_control"]
            system.contextual_awareness = self._current_capabilities["contextual_awareness"]
            system.personalization = self._current_capabilities["personalization"]
            system.multimodal_interaction = self._current_capabilities["multimodal_interaction"]
            system.status = "operational" if self._current_learning_progress > 0 else "initializing"
            system.last_learning_session = datetime.utcnow() if self._current_learning_progress > 0 else None
            system.last_self_improvement = datetime.utcnow() if self._current_learning_progress > 0 else None
        ...
        while True:
            try:
                ...
                if self.db:
                    ...
                    print(f"💾 Data persisted: {self._current_learning_progress:.2f} progress, {self._current_neural_connections} connections, {self._current_knowledge_base_size} knowledge")
...
'''
        ...
        # Write the completely new persistent service file
        with open(service_file, 'w') as f:
            f.write(persistent_service_content)
        
        print("✅ Project Warmaster service updated with persistent database storage")
        print("💾 Live data will now persist in Neon database")
        print("🔄 Data will be loaded on startup and saved every 30 seconds")
        print("📊 All progress will be maintained across service restarts")
...
if __name__ == "__main__":
    asyncio.run(fix_live_data_persistence()) 