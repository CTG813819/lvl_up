...
# EC2 Shared Token Limits Deployment Script
# This script runs directly on the EC2 instance to implement shared token limits

set -e

echo "🚀 Deploying Shared Token Limits System on EC2"
echo "=============================================="
...
# Step 5: Update the main app to include shared limits routes
...
# Step 6: Update the token usage service to use shared limits
print_status "Updating token usage service..."

# Create a backup of the original service
cp app/services/token_usage_service.py app/services/token_usage_service.py.backup

# Update the token usage service to integrate with shared limits
cat > app/services/token_usage_service_shared.py << 'EOF'
"""
...
async def test_shared_limits():
    ...
    try:
        ...
        from app.services.token_usage_service_shared import updated_token_usage_service
        recommendation = await updated_token_usage_service.get_provider_recommendation("imperium")
...
# Step 10: Restart the backend service
...
echo "Testing /api/shared-limits/notifications..."
curl -s http://localhost:8000/api/shared-limits/notifications | head -10
...
echo "🔔 Check notifications: http://localhost:8000/api/shared-limits/notifications"
