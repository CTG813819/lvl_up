#!/usr/bin/env python3
"""
Analyze Dart Files for Endpoint Usage
Check which endpoints are being used in Dart files and verify they're working
"""
...
class DartEndpointAnalyzer:
    ...
    def extract_endpoints_from_dart_file(self, file_path):
        """Extract all API endpoints from a Dart file"""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
            
            # Find all Uri.parse calls with API endpoints
            uri_pattern = r'Uri\.parse\([\'"]([^\'"]*?/api/[^\'"]*?)[\'"]\)'
            ...
            for match in matches:
                ...
                clean_endpoint = re.sub(r'/\$\{[^}]+\}', '/{id}', clean_endpoint)
    ...
    def generate_analysis_report(self):
        ...
        print("\n" + "="*60)
        print("📊 DART ENDPOINT ANALYSIS REPORT")
        print("="*60)
        ...
        with open('dart_endpoint_analysis_report.json', 'w') as f:
            json.dump(self.analysis_results, f, indent=2)
        
        print(f"\n📄 Detailed report saved to: dart_endpoint_analysis_report.json")

    def run_analysis(self):
        """Run complete analysis"""
        print("🚀 Starting Dart endpoint analysis...")
