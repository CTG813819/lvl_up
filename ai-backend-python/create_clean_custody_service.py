...
This script creates a completely clean custody_protocol_service.py
without any problematic timeout code.
"""

import os
import sys

def create_clean_custody_service():
    """Create a clean custody protocol service"""
    try:
        print("🔧 Creating clean custody protocol service...")
        
        custody_file = "app/services/custody_protocol_service.py"
        
        # Create a backup of current file
        if os.path.exists(custody_file):
            backup_file = custody_file + ".backup5"
        ...
        # Create clean content with TestCategory and TestDifficulty classes
...
class TestCategory(Enum):
    """Test categories for custody protocol tests"""
...
'''
        ...
        print("✅ Clean custody protocol service created")
        return True
        
    except Exception as e:
        print(f"❌ Error creating clean custody protocol service: {str(e)}")
        return False
...
def main():
    ...
    # Create the clean service
    if create_clean_custody_service():
        print("✅ Service created")
        
        # Restart the service
        if restart_backend_service():
            print("✅ Backend service restarted")
            print("\n🎉 Clean service created successfully!")
            print("The custody protocol service has been recreated cleanly and should now work properly.")
        else:
            print("❌ Failed to restart backend service")
    else:
        print("❌ Failed to create clean service")
