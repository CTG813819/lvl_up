...
echo "🚀 Deploying Enhanced Imperium Monitoring System to EC2 (Final Version)..."
...
mkdir -p imperium_deployment_final
cp imperium_monitoring_system.py imperium_deployment_final/
cp deploy_imperium_monitoring.py imperium_deployment_final/

# Create enhanced startup script
cat > imperium_deployment_final/start_imperium_monitoring.sh << 'EOF'
#!/bin/bash
...
cat > imperium_deployment_final/imperium-monitoring.service << 'EOF'
[Unit]
...
ExecStart=/home/ubuntu/ai-backend-python/venv/bin/python -m app.services.imperium_monitoring
Restart=always
RestartSec=10
...
cat > imperium_deployment_final/imperium_endpoints.py << 'EOF'
...
cat > imperium_deployment_final/imperium_monitoring_service.py << 'EOF'
#!/usr/bin/env python3
...
# Create test script with correct port
cat > imperium_deployment_final/test_imperium_monitoring.py << 'EOF'
#!/usr/bin/env python3
...
def test_imperium_endpoints():
    ...
    base_url = "http://localhost:4000"  # Updated to port 4000
...
tar -czf imperium_deployment_final.tar.gz imperium_deployment_final/

echo "🚀 Deploying to EC2..."
scp -i "$KEY_FILE" imperium_deployment_final.tar.gz $EC2_USER@$EC2_HOST:/home/ubuntu/
...
tar -xzf imperium_deployment_final.tar.gz
cd imperium_deployment_final

# Check if backend directory exists
if [ ! -d "/home/ubuntu/ai-backend-python" ]; then
    echo "❌ Backend directory not found. Please ensure the backend is properly deployed first."
    exit 1
fi

# Create services directory if it doesn't exist
sudo mkdir -p /home/ubuntu/ai-backend-python/app/services
sudo mkdir -p /home/ubuntu/ai-backend-python/app/routers

# Copy files to backend directory
sudo cp imperium_monitoring_service.py /home/ubuntu/ai-backend-python/app/services/
sudo cp imperium_endpoints.py /home/ubuntu/ai-backend-python/app/routers/
sudo cp start_imperium_monitoring.sh /home/ubuntu/ai-backend-python/
sudo cp test_imperium_monitoring.py /home/ubuntu/ai-backend-python/

# Make scripts executable
sudo chmod +x /home/ubuntu/ai-backend-python/start_imperium_monitoring.sh
sudo chmod +x /home/ubuntu/ai-backend-python/test_imperium_monitoring.py
...
sudo cp /home/ubuntu/imperium_deployment_final/imperium-monitoring.service /etc/systemd/system/
...
echo "🌐 Test endpoints: python test_imperium_monitoring.py"
EOF
...
source venv/bin/activate
python test_imperium_monitoring.py
...
echo "📊 Access monitoring dashboard at: http://ec2-34-202-215-209.compute-1.amazonaws.com:4000/api/imperium/monitoring"
echo "🔍 View system status at: http://ec2-34-202-215-209.compute-1.amazonaws.com:4000/api/imperium/status" 