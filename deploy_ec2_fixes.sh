...
# EC2 Database and Backend Fixes Deployment Script
# This script fixes all critical database and backend issues

set -e  # Exit on any error

echo "🔧 Starting EC2 Database and Backend Fixes Deployment..."
...
if [ -f "app/services/terra_extension_service.py" ]; then
    ...
    # Replace placeholder generate_code method
...
EOF
    ...
    sed -i '/async def generate_code/,/pass/c\'"$(cat temp_terra_fix.py)" app/services/terra_extension_service.py
...
EOF
        ...
        echo "   psql -d your_database -f create_json_function.sql"
    fi
else
