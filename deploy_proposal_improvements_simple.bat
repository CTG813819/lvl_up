@echo off
REM Deploy Proposal Improvements to EC2 (Simple Direct Copy)
...
REM Step 3: Copy files directly to EC2
echo [INFO] Step 3: Copying files to EC2...
scp -i "%SSH_KEY%" -o StrictHostKeyChecking=no "%LOCAL_PATH%\app\services\proposal_validation_service.py" "%EC2_USER%@%EC2_HOST%:%REMOTE_PATH%/app/services/"
scp -i "%SSH_KEY%" -o StrictHostKeyChecking=no "%LOCAL_PATH%\app\routers\proposals.py" "%EC2_USER%@%EC2_HOST%:%REMOTE_PATH%/app/routers/"
scp -i "%SSH_KEY%" -o StrictHostKeyChecking=no "%LOCAL_PATH%\test_proposal_validation.py" "%EC2_USER%@%EC2_HOST%:%REMOTE_PATH%/"
scp -i "%SSH_KEY%" -o StrictHostKeyChecking=no "PROPOSAL_IMPROVEMENTS_SUMMARY.md" "%EC2_USER%@%EC2_HOST%:%REMOTE_PATH%/"

if %errorlevel% equ 0 (
    echo [SUCCESS] Files copied to EC2
) else (
    echo [ERROR] Failed to copy files to EC2
    exit /b 1
)

REM Step 4: Deploy on EC2
echo [INFO] Step 4: Deploying on EC2...
ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no "%EC2_USER%@%EC2_HOST%" "echo '🔧 Deploying proposal improvements on EC2...' && cd /home/ubuntu/ai-backend-python && echo '🐍 Activating virtual environment...' && source venv/bin/activate && echo '📦 Installing dependencies in virtual environment...' && pip install scikit-learn pandas numpy joblib requests && echo '🧪 Testing proposal validation service...' && python test_proposal_validation.py && echo '🔄 Restarting backend service...' && sudo systemctl restart ai-backend-python && sleep 10 && echo '📊 Checking service status...' && sudo systemctl status ai-backend-python --no-pager && echo '🔍 Testing new endpoints...' && curl -s http://localhost:8000/api/proposals/validation/stats && echo '✅ Deployment completed on EC2'"
...
echo [INFO] Step 5: Testing the deployment...
...
REM Step 6: Generate deployment report
echo [INFO] Step 6: Generating deployment report...
(
