...
echo "🚀 Deploying Enhanced Imperium Monitoring System to EC2 (Fixed Version)..."
...
mkdir -p imperium_deployment_fixed
cp imperium_monitoring_system.py imperium_deployment_fixed/
cp deploy_imperium_monitoring.py imperium_deployment_fixed/

# Create enhanced startup script
cat > imperium_deployment_fixed/start_imperium_monitoring.sh << 'EOF'
#!/bin/bash
...
cat > imperium_deployment_fixed/imperium-monitoring.service << 'EOF'
[Unit]
...
cat > imperium_deployment_fixed/imperium_endpoints.py << 'EOF'
...
cat > imperium_deployment_fixed/imperium_monitoring_service.py << 'EOF'
#!/usr/bin/env python3
...
# Create test script
cat > imperium_deployment_fixed/test_imperium_monitoring.py << 'EOF'
#!/usr/bin/env python3
...
tar -czf imperium_deployment_fixed.tar.gz imperium_deployment_fixed/

echo "🚀 Deploying to EC2..."
scp -i "$KEY_FILE" imperium_deployment_fixed.tar.gz $EC2_USER@$EC2_HOST:/home/ubuntu/
...
tar -xzf imperium_deployment_fixed.tar.gz
cd imperium_deployment_fixed

# Check if backend directory exists and create structure if needed
