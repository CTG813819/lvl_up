...
Write-Host "📁 Step 1: Copying fix_all_fake_data_corrected.py to EC2 ai-backend-python directory..." -ForegroundColor Green
# Use scp to copy the file to the correct directory
$scpCommand = "scp -i `"$KEY_PATH`" fix_all_fake_data_corrected.py ${EC2_USER}@${EC2_HOST}:/home/ubuntu/ai-backend-python/"
...
if [ ! -f "fix_all_fake_data_corrected.py" ]; then
    echo "❌ fix_all_fake_data_corrected.py not found"
    exit 1
fi
...
python3 fix_all_fake_data_corrected.py
...
$tempScript = "temp_comprehensive_fix_corrected.sh"
